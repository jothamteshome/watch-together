...
export default function PlaylistVideoList({ videos, currentIndex, onVideoSelect, fetchVideoInfo }: PlayistVideoListProps) {
        ...
        <div className="w-full flex flex-col overflow-y-auto overscroll-none rounded-b-lg">
            {
                ...
                (
                    <PlaylistVideo ref={i === currentIndex ? currentVideoRef : null} key={`${videoUrl}-${i}`} watching={i === currentIndex} videoUrl={videoUrl} index={i} onVideoSelect={() => onVideoSelect(i)} fetchVideoInfo={fetchVideoInfo} />
                ))
            }
        </div>
    );
